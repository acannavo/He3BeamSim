...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    // ------------------------------------------------------------
    // He-4 gas (MANUAL DEFINITION with pressure from config)
    // ------------------------------------------------------------

    const G4double pressure_Torr = cfg.GasPres();
    const G4double pressure_Pa = pressure_Torr * 133.322 * pascal;
    const G4double temperature   = 293.15 * kelvin;

    // Ideal gas law: rho = P*M / (R*T)
    const G4double molarMass = 4.0026 * g/mole;
    const G4double R = CLHEP::k_Boltzmann * CLHEP::Avogadro;

    const G4double density =
        (pressure_Pa * molarMass) / (R * temperature);

    // Manual element
    G4Element* elHe4 = new G4Element("Helium4", "He4", 2, molarMass);

    // Manual material
    G4Material* He4gas_manual = new G4Material(
        "He4gas_manual",
        density,
        1,
        kStateGas,
    ...
    He4gas_manual->AddElement(elHe4, 1);

    G4cout << "\n[Geometry] He-4 gas (MANUAL):\n"
           << "  Pressure : " << pressure_Torr << " Torr (" << pressure_Pa/pascal << " Pa)\n"
    ...
    auto* logicGas = new G4LogicalVolume(solidGas, He4gas_manual, "He4Gas");
